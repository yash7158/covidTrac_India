...
if y!="coronavirus":
    ...
    for url in search(query, tld="co.in", num=1, stop = 1, pause = 2):
        webbrowser.open("https://google.com/search?q=%s" % query)
if r.recognize_google(audio)=="yo":
    ...
    def openNewWindow():
        ...
        newWindow.config(bg="Dodgerblue2")
        newWindow.title("Coronavirus Self Test")
    #questions
        r= Text(newWindow,height="13",width="130",bg="Dodgerblue2",font=("ALGERIAN",18),fg="black")
        ...
        r.pack(side="top")
    #button
        b1=Button(newWindow,text="YES",font=("Monotype Corsiva",20),activeforeground="green",command=lambda lm=output0:[opens(),command(lm),newWindow.destroy()] )
        b2=Button(newWindow,text="NO",font=("Monotype Corsiva",20),activeforeground="red",command=lambda mp=output1:[opens(),command(mp),newWindow.destroy()] )
        b1.pack(side="left")
        b2.pack(side="right")
    ...
    def opens():
        ...
        s.config(bg="Dodgerblue2")
        s.title("Coronavirus Self Test")
    #question
        z= Text(s,height="13",width="130",bg="Dodgerblue2",font=("Algerian",18),fg="black")
        ...
        z.pack(side="top")
        b3=Button(s,text="YES",activeforeground="red",font=("Monotype Corsiva",20),command=lambda ln=output0:[openj(),command(ln),s.destroy()] )
        b4=Button(s,text="NO",activeforeground="green",font=("Monotype Corsiva",20),command=lambda ld=output1:[openj(),command(ld),s.destroy()] )
        b3.pack(side="left")
        b4.pack(side="right")
    #4 window  
    def openj():
        ...
        j.config(bg="Dodgerblue2")
        j.title("Coronavirus Self Test")
    
        jz= Text(j,height="13",width="130",bg="dodgerblue2",font=("ALGERIAN",20),fg="black")
        jz.insert(INSERT, "Have you travelled or came from an outside country?,\n(like USA,Italy,Russia,China)")
        jz.pack(side="top")
        b5=Button(j,text="YES",font=("Monotype Corsiva",20),activeforeground="green",command=lambda vg=output0:[openy(),command(vg),j.destroy()] )
        b6=Button(j,text="NO",font=("Monotype Corsiva",20),activeforeground="red",command=lambda vm=output1:[openx(),command(vm),j.destroy()])
        b5.pack(side="left")
        b6.pack(side="right")
    ...
    def openm():
        ...
        m.config(bg="Dodgerblue2")
        m.title("Coronavirus Self Test")
        #Q1
        w= Text(m,height="12",width="100",bg="Dodgerblue2",font=("Algerian",20),fg="black") 
        ...
        output0="yes"
        output1="no"
        b=Button(m,text="YES",font=("Monotype Corsiva",20),activeforeground="red",command=lambda h=output0:[openNewWindow(),command(h),m.destroy()])
        bb=Button(m,text="NO",font=("Monotype Corsiva",20),activeforeground="green",command=lambda i=output1:[openNewWindow(),command(i),m.destroy()])
        b.pack(side="left")
        bb.pack(side="right")
    ...
    def runst():
        ...
        my_url = 'https://en.wikipedia.org/wiki/COVID-19_pandemic_in_India'
        res = requests.get(my_url)
        page_html = res.text
        page_soup = soup(page_html, 'html.parser')
